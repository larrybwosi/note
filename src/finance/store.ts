...
export const enhancedFinanceUtils = {
  ...
  calculateBudgetAllocations: (config: BudgetConfig) => {
    ...
    switch (selectedRule) {
      ...
      case BUDGET_RULES.RULE_70_20_10:
        return {
          expenses: monthlyIncome * 0.7,
          savings: monthlyIncome * 0.2,
          debtOrDonation: monthlyIncome * 0.1,
        };
      case BUDGET_RULES.CUSTOM:
        return config.customRules?.reduce(
          (acc, rule) => ({
            ...acc,
  ...
  calculateGuiltFreeBalance: (store: EnhancedFinanceStore, currentDate = new Date()) => {
    const monthStart = startOfMonth(currentDate);
    const monthEnd = endOfMonth(currentDate);

    // Get all transactions within current month
    const monthlyTransactions = Object.values(store.transactions).filter((transaction) =>
    ...
    const allocations = enhancedFinanceUtils.calculateBudgetAllocations(store.budgetConfig);
  ...
  detectUnusualSpending: (
    ...
    const unusualSpending = store.categories
      .map((category) => {
        ...
        const percentageIncrease = (currentMonthSpending - lastMonthSpending) / lastMonthSpending;
  ...
  calculateCategorySpending: (store: EnhancedFinanceStore, categoryId: string, month: Date) => {
...
export const createEnhancedFinanceStore = () =>
  observable(

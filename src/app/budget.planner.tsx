...
import { BudgetRuleType, CustomRule, } from 'src/store/finance/types';
import { BudgetRuleTypeSchema } from 'src/store/finance/src/types';

const BudgetPlannerScreen: React.FC = () => {
  const [selectedRule, setSelectedRule] = useState<BudgetRuleType>(BudgetRuleTypeSchema.enum['50/30/20']);
  ...
  const handleRuleChange = (rule: BudgetRuleType) => {
    ...
    if (rule === BudgetRuleTypeSchema.enum.CUSTOM) {
            ...
            {budgetData.map((item) => (
              <View key={item.key} className="items-center">
                <View
                  ...
                  style={{ backgroundColor: item.svg.fill.split('#')[1] }}
                />
                <Text className="text-sm font-rmedium text-gray-700 dark:text-gray-300">
                  {item.label}
                </Text>
                <Text className="text-sm font-rbold text-gray-900 dark:text-white">
                  {item.amount}%
                </Text>
              </View>
            ))}
...
{/* 
        ...
        {showCustomRuleForm && <CustomRuleForm onAddRule={handleAddCustomRule} />}

        {selectedRule === BudgetRuleTypeSchema.enum.CUSTOM && customRules.length > 0 && (
          <BudgetBreakdown rules={customRules} />
        )}

        {selectedRule === BudgetRuleTypeSchema.enum.CUSTOM && (
          <TouchableOpacity

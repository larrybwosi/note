...
const HomeScreen = observer(() => {
  ...
  useEffect(() => {
    ...
    welcomeScale.value = withSequence(withSpring(1.1), withSpring(1));
  ...
  const FocusTipCard = observer(({ icon, title, description }: any) => (
    <View
      className={`dark:bg-gray-900 dark:border-gray-800 bg-gray-50 border-gray-100 
      p-4 rounded-xl mb-3 border`}
    >
      <View className="flex-row items-center mb-2">
        <Text className="text-2xl mr-2">{icon}</Text>
        <Text className={`dark:text-white text-gray-800 font-plregular`}>{title}</Text>
      </View>
      ...
      >
        <LinearGradient colors={headerGradient} className="px-6 pt-16 pb-10 rounded-b-3xl">
          <Animated.View style={welcomeAnimStyle} className="space-y-3">
            ...
            <Text className="text-gray-100 text-base font-aregular">{getGreeting.get()}</Text>
            <View className={`mt-4 bg-white/10 p-4 rounded-2xl backdrop-blur-lg`}>
              <Text className="text-white font-rmedium">{dailyQuote.get().quote}</Text>
              <Text className="text-gray-200 mt-2 font-plregular">- {dailyQuote.get().author}</Text>
            </View>
          </Animated.View>
        </LinearGradient>
        ...
        <View className="px-6 mt-8 mb-8 dark:bg-gray-900">
          <Text
            className={`text-2xl font-rbold mb-2 ${isDark ? 'dark:text-white' : 'text-gray-800'}`}
          >
            Today's Insights
          </Text>
          ...
          >
            {focusTips.get().map((tip, index) => (
              <Memo key={index}>{() => <FocusTipCard {...tip} />}</Memo>
            ))}
          </Animated.View>
        </View>
      </ScrollView>

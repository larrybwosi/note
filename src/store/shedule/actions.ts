...
const canPostpone = (item: ScheduleItem): boolean => {
  ...
  return (
    ...
    !item.blockedBy?.some(id => 
      !scheduleStore.get().items.find(i => i.id === id)?.completed
    )
  )
}
...
const calculateProductiveHours = (): string[] => {
  const completedItems = scheduleStore.get().items.filter(
...
const updatePerformanceMetrics = (): void => {
  ...
  const delays = postponed.flatMap(item =>
    item.postponements.map(p =>
      (p.newDate.getTime() - p.originalDate.getTime()) / (1000 * 60)
    )
  )
  ...
  const focusTime = completed.reduce(
...
export const getOverdueTasks = (): ScheduleItem[] => {
  const now = new Date()
  return scheduleStore.get().items.filter(
    item => !item.completed && isBefore(item.endDate, now)
  )
}
...
export const getUpcomingTasks = (days: number = 7): ScheduleItem[] => {
  ...
  return scheduleStore.get().items.filter(
    item => 
      !item.completed && 
      isAfter(item.startDate, now) && 
      isBefore(item.startDate, futureDate)
  )
}

export const getTasksByPriority = (priority: ScheduleItem['priority']): ScheduleItem[] => {
  return scheduleStore.get().items.filter(
    item => !item.completed && item.priority === priority
  )
}
...
export const restoreDeletedItem = (id: number) => {
  ...
  if (item) {
    ...
    scheduleStore?.deletedItems?.set(scheduleStore?.deletedItems?.get()?.filter(item => item.id !== id))
  }
}
...
export const restoreCompletedItem = (id: number) => {
  ...
  if (item) {
    ...
    scheduleStore?.items?.push(restoredItem)
    scheduleStore?.completedItems?.set(scheduleStore?.completedItems?.get()?.filter(item => item.id !== id))
  }
}

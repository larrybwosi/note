...
const actions = {
  ...
  getBalance: () => {
    // return store.budgets.reduce((total, b) => total + use$(b.amount), 0) - actions.getTotalSpent();
    const totalIncome = store.transactions.map((t) => use$(t.type) === 'income' ? use$(t.amount) : 0).reduce((total, amount) => total + amount, 0);
    const totalExpense = store.transactions.map((t) => use$(t.type) === 'expense' ? use$(t.amount) : 0).reduce((total, amount) => total + amount, 0);
    return totalIncome - totalExpense;
    // return store.transactions.map((t) => use$(t.type) === 'expense' ? -use$(t.amount) : use$(t.amount)).reduce((total, amount) => total + amount, 0);
  }
};

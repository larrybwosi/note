...
const Slider: React.FC<SliderProps> = ({
  ...
  const getOffsetFromValue = useCallback((val: number): number => {
    'worklet';
    const range = maximumValue - minimumValue;
    ...
    (currentValue) => {
      if (!isSliding.value && containerWidth.value > 0) {
        ...
        translateX.value = withSpring(
          inverted ? containerWidth.value - offset : offset,
  ...
  const gesture = Gesture.Pan()
    ...
    .onUpdate((event) => {
      const newTranslateX = prevTranslateX.value + event.translationX * (I18nManager.isRTL ? -1 : 1);
  ...
  const animatedTrackStyle = useAnimatedStyle(() => ({
    width: inverted
      ? containerWidth.value - translateX.value
      : translateX.value,
  }));
